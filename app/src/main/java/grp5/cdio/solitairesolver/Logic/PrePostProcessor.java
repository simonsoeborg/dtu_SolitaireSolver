...
public class PrePostProcessor {
     ...
     */
    static ArrayList<PrePostProcessorResult> nonMaxSuppression(ArrayList<PrePostProcessorResult> boxes, int limit, float threshold) {

        // Do an argsort on the confidence scores, from high to low.
        Collections.sort(boxes,
            new Comparator<PrePostProcessorResult>() {
                @Override
                public int compare(PrePostProcessorResult o1, PrePostProcessorResult o2) {
                    return o1.score.compareTo(o2.score);
                }
            });

        ArrayList<PrePostProcessorResult> selected = new ArrayList<>();
        ...
        for (int i=0; i<boxes.size() && !done; i++) {
            if (active[i]) {
                PrePostProcessorResult boxA = boxes.get(i);
                selected.add(boxA);
                if (selected.size() >= limit) break;

                for (int j=i+1; j<boxes.size(); j++) {
                    if (active[j]) {
                        PrePostProcessorResult boxB = boxes.get(j);
    ...
    public static ArrayList<PrePostProcessorResult> outputsToNMSPredictions(float[] outputs, float imgScaleX, float imgScaleY, float ivScaleX, float ivScaleY, float startX, float startY) {
        mOutputColumn = mClasses.length + 5;
        mOutputRow = mInputHeight * mInputWidth;
        ArrayList<PrePostProcessorResult> results = new ArrayList<>();
        for (int i = 0; i< mOutputRow; i++) {
            if (outputs[i* mOutputColumn +4] > mThreshold) {
                ...
                PrePostProcessorResult result = new PrePostProcessorResult(cls, outputs[i*mOutputColumn+4], rect);
                results.add(result);
            }
        }
